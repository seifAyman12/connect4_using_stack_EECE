...
/*****************************  LINKED LIST DATA STRUCTURE ADD BY THE TEAM *******************************************/
struct linked_list 
{
    int data ;
...
void push(struct linked_list** n, int new_data)
{
...
void deleteNode(struct linked_list** head_ref, int key)
{
...
struct stack {// *&*&* this point will be updated by (seif,ramy,zyad)
...
void playGame(char gameBoard[][BOARD_HORIZONTAL]) { // Routine that starts the game
    ...
    int namesEntered = 0; // Flag to determine if the names are entered.
    int column = 0;
    int theRow = 0;
...
int makeMove(char gameBoard[][BOARD_HORIZONTAL]) { // Routine that allows Player X to make a move
   ...
   static struct linked_list* player1;// the list added by the team.
   static struct linked_list* player2;//by the team
   ...
   char popMoveFlag;
   char saveGameFlag;
   char buffer[20];// by the team , to avoid string as input
   char reviwer[8] = {0,0,0,0,0,0,0,0} ;// by the team , to check if the clomuns are full
   char detector = 0 ;// by the team , to check all the points of the game if they are full
/***********************************************************************************************************************************/
   do {
       counter++;
       /********************************** to check all the clomuns *******************************************/
       for(int i=1 ; i<8 ; i++)
       {
           if(reviwer[i]==6)
           {
       ...
       if(detector==7)
       {
           printf("the all columns are full , GAME OVER.\n");
           startGame();
       }
       /***********************************************************************************************************/
       ...
       if(counter % 2 == 0) { // If the counter is divisible by 2, change player.
           ...
           printf("\n The last Player %s moves are (the last at most left) {", playerOneName);// show the last moves of player1
           printlist(player1);// print the list of player1 
           printf("}\n");
           scanf("%s", buffer);

           reviwer[(buffer[0]-48)]++;// incerement the elment in the array indexed by the input clomun 

           while(reviwer[(buffer[0]-48)]>6)
           {
               printf("\n not valid , this column is full , you should chose anthor one ");
               scanf("%s", buffer);
           }
           ...
           token = 'O';
           push((&player1),(buffer[0]-48));// after all the check operation push the input move to the linked list.
       }

       else {
           printf("\n Player %s it's your turn now. Hit 0 to Save Game .", playerOneName); // Prompts the player to make a move.
           printf("\n The last Player %s moves are (the last at the most left) {", playerTwoName);// show the last moves of player 2
           printlist(player2);// print the list of player1 
           printf("}\n");
           scanf("%s", buffer);
           ...
           max_Horizon[theColumn]++ ; 
           token = 'X';
           push((&player2),(buffer[0]-48));// after all the check operation push the input move to the linked list.
       }
       ...
       if((buffer[0] - 48) == 0) { // If the column entered is 0.
...
int isColumnFull(char gameBoard[][BOARD_HORIZONTAL], int random_move) {//i can not understand
...
void saveGame(char gameBoard[][BOARD_HORIZONTAL]) { // i can not understand
     // Routine that saves the game to a file so the game can be played later (loaded)
...
void startGame() {
 ...
 do {
     ...
     printf("What is youer choice (0;0) !!!\n");
